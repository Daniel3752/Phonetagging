...
globalThis.fetch = async (url, opts) => {
  ...
  if (u.includes('/rest/private/devices/search')) {
    hwCalls.push('search');
    return Response.json({ status: 'OK', data: hwDevices });
  }
...
hwDevices = [{ id: 7, name: 'phone-a', configurationId: '10' }, { id: 8, name: 'phone-b', configurationId: '10' }];
...
check('device 7 moved to config 20', hwDevices.find((d) => d.id === 7).configurationId === '20');
...
check('device 7 back to config 10', hwDevices.find((d) => d.id === 7).configurationId === '10');
...
check('healthy device still updated', hwDevices.find((d) => d.id === 8).configurationId === '20');
