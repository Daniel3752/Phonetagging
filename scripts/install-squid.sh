...
# The in-app picture allowlist (app_media_on in squid.conf): the tunnel addresses whose rung may see
# a pinned app's own pictures. Must exist before squid starts — squid refuses to start on a missing
# acl file — and starts as placeholder-only, i.e. pictures off everywhere, which is the safe way
# round until the first sync runs.
if [[ ! -f /etc/squid/app-media-on.txt ]]; then
  printf '# Phones whose rung may see in-app pictures. Generated by sync-media-on.sh.\n10.66.0.255\n' \
    > /etc/squid/app-media-on.txt
  chmod 644 /etc/squid/app-media-on.txt
fi
...
/usr/local/bin/sync-media-on.sh || echo "    initial media-on sync failed — cron will retry"
