...
for (const host of INFRA) {
  rows.push({ hostname: host, level: 1, reason: 'Infrastructure: fonts/scripts/assets', isDoorway: 0 });
}
for (const site of SITES) {
  for (const host of site.hosts) {
    rows.push({ hostname: host.toLowerCase(), level: site.level, reason: site.reason, isDoorway: 0 });
  }
}
...
const values = finalRows.map((r) => {
  ...
  const verdict = r.level >= 5 ? 'blocked' : 'clean';
  return `  ('${hash}', 'https://${esc(r.hostname)}/', '${esc(r.hostname)}', 'host', '${verdict}', '${esc(r.reason)}', 'operator', ${now}, ${r.level}, ${r.isDoorway})`;
}).join(',\n');
...
-- still wins. Infrastructure (fonts/scripts/assets) is level 1 so it resolves at every rung.
